...
**禁止重排序**：为了性能优化，JMM 在不改变正确语义的前提下，会允许编译器和处理器对指令序列进行重排序。JMM 提供了内存屏障阻止这种重排序。 在 volatile 写前插入一个 StoreStore 屏障 在 volatile 写后插入一个 StoreLoad 屏障。 在 volatile 读后面插入一个 LoadLoad 屏障和 LoadStore 屏障。
...
**初始化**阶段会将之前赋的默认值替换成真正的初始值，也就是编译器会将类文件声明的静态赋值变量和静态代码块合并生成类构造器方法并调用。
...
- 传统数据库模糊查询不走索引，当数据量很大时，查询效率非常低下
- Es搜索可以对搜索词进行自动分词等处理，更容易查询到相关的内容
...
但是，Lucene 仅仅只是一个库，而且非常复杂，不支持分布式。

ElasticSearch 内部使用 Lucene 做索引和搜索，Es索引的每一个分片其实就相当于一个 Lucene。Es隐藏了 Lucene 的复杂性，取而代之的是提供了一套简单通用的 RestFul API。且支持分布式存储
...
创建索引时，Es会把每个文档拆分为独立的词，然后创建一个包含所有词的词条列表，记录了每个词条出现在哪些文档中

搜索时，可以直接根据词，快速定位到相应的文档。



#### 基础查询操作

- Match：适用于全文搜索，Es会根据 Match 的词进行分值计算，match 多个词时相当于对每个词分别进行查询，多个词之间是 or 的关系
- Term：适用于精准匹配，term 查询的词，会被视为一个整体，且区分大小写
- bool：组合查询，包括四种操作符
  - must：必须匹配，
  - must_not：必须不能匹配
  - should：选择性匹配，至少满足一条
  - filter：必须匹配，与 must 不同的事，filter 不贡献算分



#### 深分页问题

**滚动搜索 scroll**：查询时，初始化一个带有 scroll 参数的搜索请求，这个请求会放回一个 scroll id，用于后续的滚动，接着就可以使用 scroll id 来获取下一批的数据了

**Search_after**：第一次查询时，定义一个排序规则，在后续的查询中，可以使用上一次查询接口的最后一条记录的排序值

![image-20250813094452413](C:\Users\25692\AppData\Roaming\Typora\typora-user-images\image-20250813094452413.png)
...
3.如果客户端从大多数实例获取锁成功，并且再次获取当前时间戳T2，只有 T2 - T1 小于锁的过期时间才会加锁成功
